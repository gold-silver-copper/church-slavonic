...
//! (96..119; see `church_slavonic_core_legacy::schema`)). A cell the row does not
...
//! sort of each lemma's emitted forms (see `extractor_legacy::assign`): the bare key
...
//!    row of its first choices (`extractor_legacy::assign`).

use church_slavonic_core_legacy::ChurchSlavonicCore;
pub use church_slavonic_core_legacy::grammar::*;
pub use church_slavonic_core_legacy::orthography;
use church_slavonic_core_legacy::orthography::{realise, strip_marks};
...
// `church_slavonic_core_legacy::schema`.
use church_slavonic_core_legacy::schema::{
...
/// [`church_slavonic_core_legacy::sense_key::split`] — the single owner of the key
/// format, shared with the extractor's generator.
fn canonical_sense_suffix_base(word: &str) -> Option<&str> {
    church_slavonic_core_legacy::sense_key::split(word).map(|(base, _)| base)
}
...
impl ChurchSlavonic {
    ...
    /// # Examples
    /// ```rust
    /// use church_slavonic_legacy::{Case, ChurchSlavonic, Number, Recension};
    ///
    /// assert_eq!(
    ...
    pub fn noun(word: &str, case: &Case, number: &Number, recension: &Recension) -> String {
        ...
        // fact.
        let engine_fact = std::iter::once(church_slavonic_core_legacy::schema::NOUN_ACCENT_CELL)
            .chain(church_slavonic_core_legacy::schema::NOUN_SHAPE_SOURCE_CELLS)
        ...
        if let Some((_, style)) = engine_fact {
            ...
            return restyle(
                orthography::realise(
                    &church_slavonic_core_legacy::resolution::noun_fact_fallback(
                        &realised,
                        recension,
    ...
    /// # Examples
    /// ```rust
    /// use church_slavonic_legacy::{Case, ChurchSlavonic, Degree, Gender, Number, Recension};
    ///
    /// assert_eq!(
    ///     ChurchSlavonic::adj(
    ...
    pub fn adj(
        ...
        if let Some(cell) = adj_cell(case, number, gender, degree)
            && let Some((_, style)) = attested_cell(
                ...
                base,
                recension,
                Some(church_slavonic_core_legacy::schema::ADJ_ACCENT_CELL),
                &get,
            )
        {
            let fact = |i: usize| -> Option<String> {
            ...
            return restyle(
                orthography::realise(
                    &church_slavonic_core_legacy::resolution::adj_fact_fallback(
    ...
    /// # Examples
    /// ```rust
    /// use church_slavonic_legacy::{ChurchSlavonic, Form, Number, Person, Recension, Tense};
    ///
    /// assert_eq!(
    ///     ChurchSlavonic::verb(
    ...
    pub fn verb(
        ...
        // engine — `church_slavonic_core_legacy::resolution` owns the order.
        ...
        let accent = attested_cell(
            ...
            base,
            recension,
            Some(church_slavonic_core_legacy::schema::VERB_ACCENT_CELL),
            &get,
        );
            ...
            && let Some(cell) = verb_cell(person, number, tense, form)
        {
            ...
            return restyle(
                orthography::realise(
                    &church_slavonic_core_legacy::resolution::verb_fact_fallback(
                        &realised, recension, cell, &fact,
                    ),
                    recension,
                ),
    ...
    pub fn l_participle(
        ...
        let get = |w: &str| get_verb(&format!("{}:{w}", tag(recension)));
        let base = base_lemma(word, |w| ci_lookup(w, recension, get).is_some());
        let cell = church_slavonic_core_legacy::schema::l_participle_cell(gender, number);
    ...
    pub fn participle(
        ...
        // `church_slavonic_core_legacy::resolution` owns the order.
    ...
    pub fn npron(
        ...
        let base = base_lemma(word, |w| ci_lookup(w, recension, get).is_some());
        let cell = church_slavonic_core_legacy::schema::npron_cell(gender, number, case);
        if let Some((c, style)) = attested_cell(word, base, recension, Some(cell), get) {
            return restyle(c.to_string(), style);
        }
        rule_with_case(base, recension, |w| {
    ...
    /// # Examples
    /// ```rust
    /// use church_slavonic_legacy::{Case, ChurchSlavonic, Gender, Number, Person, Recension};
    ///
    /// assert_eq!(
    ...
    /// # Examples
    /// ```rust
    /// use church_slavonic_legacy::{Case, ChurchSlavonic, Recension};
    ///
    ...
    /// # Examples
    /// ```rust
    /// use church_slavonic_legacy::ChurchSlavonic;
    ///
...
mod rule_table_sync_tests {
    ...
    use super::*;
    use church_slavonic_core_legacy::orthography::comparison_key;
            ...
            for i in 0..38 {
                ...
                if !attested.is_empty()
                    ...
                    && same(
                        ...
                        attested,
                        &rule(lemma, &r, |l| {
                            church_slavonic_core_legacy::resolution::verb_fact_fallback(l, &r, i, &fact)
                        }),
                    )
                {
