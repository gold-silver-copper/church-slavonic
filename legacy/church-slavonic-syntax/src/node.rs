...
pub use church_slavonic_legacy::{Case, Degree, Form, Gender, Number, Person, Series, Tense, Voice};
...
/// the given recension.
pub fn render(node: &Node, recension: &church_slavonic_legacy::Recension) -> Result<String, TreeError> {
...
fn walk(
    node: &Node,
    recension: &church_slavonic_legacy::Recension,
    out: &mut String,
    glue_next: &mut bool,
) -> Result<(), TreeError> {
...
mod tests {
    ...
    fn verbatim_wrap_round_trips_the_pitfall_verse() {
        ...
        let rendered =
            render(&tree, &church_slavonic_legacy::Recension::Synodal).expect("renders");
    ...
    fn analyzed_leaves_render_through_the_crate() {
        let syn = church_slavonic_legacy::Recension::Synodal;
        let read = |text: &str| {
    ...
    fn genesis_1_1_lifts_and_round_trips() {
        ...
        assert_eq!(
            render(&node, &church_slavonic_legacy::Recension::Synodal).expect("renders"),
