...
use church_slavonic_core_legacy::grammar::*;
use church_slavonic_core_legacy::grammar::{Series as GSeries, Voice as GVoice};
use church_slavonic_core_legacy::orthography::{
...
pub fn pronoun_attested_matches(
    ...
    attested_matches(exact, recension, attested, predicted)
        || (!exact
            && *recension == Recension::Synodal
            && church_slavonic_core_legacy::orthography::number_mark_equivalent(attested, predicted))
}
...
fn has_consonant_kamora(surface: &str) -> bool {
    ...
    for c in surface.nfd() {
        if c == '\u{311}' && previous.is_some_and(|p| !church_slavonic_core_legacy::orthography::is_vowel_letter(p)) {
            return true;
        }
...
fn infer_verb_override(
    ...
    let realised = church_slavonic_core_legacy::orthography::realise(lemma, recension);
    let finite = |class: &str, present: &str, i: usize| {
        ...
        church_slavonic_core_legacy::orthography::realise(
            &church_slavonic_core_legacy::resolution::verb_fact_fallback(&realised, recension, i, &fact),
            recension,
        )
    };
    let plain = attested
...
fn resolved_cell(
    ...
    use church_slavonic_core_legacy::resolution as res;
    let raw = match pos {
    ...
    church_slavonic_core_legacy::orthography::realise(&raw, recension)
}
...
fn shape_sources(pos: Pos) -> &'static [usize] {
    use church_slavonic_core_legacy::schema as sch;
    match pos {
...
fn infer_accent_pattern(
    ...
    use church_slavonic_core_legacy::orthography::{realise, stressed_vowel_index, vowel_count};
...
pub fn finalize(lexemes: &Lexemes) -> Tables {
    ...
    for (key, observations) in lexemes {
        ...
        if recension == Recension::Synodal
            && church_slavonic_core_legacy::orthography::is_accented(&key.lemma)
        {
            use church_slavonic_core_legacy::orthography::{is_accented, vowel_count};
            for cand in &mut candidates {
                cand.noise = cand
                    .forms
                    .iter()
        ...
        if let Some((_, fact_range)) = fact_geometry(key.pos) {
            let form_end = fact_range.start;
            let realised = church_slavonic_core_legacy::orthography::realise(&key.lemma, &recension);
...
mod tests {
    ...
    fn alypy_merges_into_the_polyakov_observation_and_disagreements_are_counted() {
        ...
        assert_eq!(
            t.noun[0].1[church_slavonic_core_legacy::schema::NOUN_ACCENT_CELL],
            "s1"
        );
