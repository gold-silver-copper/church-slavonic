...
use church_slavonic_core_legacy::orthography::{comparison_key, realise};
pub use church_slavonic_core_legacy::verb::Conj;

pub use church_slavonic_core_legacy::schema::{
...
/// through [`church_slavonic_core_legacy::resolution::verb_fact_fallback`], the
...
mod tests {
    ...
    fn cell_indices_tile_each_row_exactly_once() {
        ...
        seen[church_slavonic_core_legacy::schema::NOUN_ACCENT_CELL] = true;
        ...
        seen[church_slavonic_core_legacy::schema::ADJ_ACCENT_CELL] = true;
        assert!(seen.iter().all(|s| *s));
        assert_eq!(
            adj_cell(
                &Case::Nominative,
        ...
        for i in [
            PRESENT_STEM_CELL,
            VERB_CLASS_CELL,
            church_slavonic_core_legacy::schema::VERB_ACCENT_CELL,
        ] {
            assert!(!seen[i]);
            seen[i] = true;
        }
        ...
        use church_slavonic_core_legacy::schema::{PronounCell, pronoun_features};
        assert_eq!(
