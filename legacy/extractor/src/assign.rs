...
//! [`church_slavonic_core_legacy::sense_key`], the single owner shared with the
...
/// The emitted key format lives in [`church_slavonic_core_legacy::sense_key`] — the
/// single owner shared with the runtime.
pub use church_slavonic_core_legacy::sense_key::make_key;
...
pub fn split_key(key: &str) -> Option<(&str, u32)> {
    let (base, digits) = church_slavonic_core_legacy::sense_key::split(key)?;
    let suffix = digits
        .parse()
