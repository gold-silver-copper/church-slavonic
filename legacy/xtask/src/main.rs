...
//! regenerates every table from the sources alone (see `extractor_legacy::assign`), so
...
use extractor_legacy::bootstrap::audit_tables;
use std::env;
use std::error::Error;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.next().as_deref() {
        ...
        Some("build-treebank") => church_slavonic_syntax_legacy::treebank::run(true),
        Some("check-treebank") => church_slavonic_syntax_legacy::treebank::run(false),
...
fn check_registry() -> Result<(), Box<dyn Error>> {
    let generated_dir = workspace_root()?.join("legacy/church-slavonic/generated");
...
fn run_extractor(args: Vec<String>, checks_only: bool) -> Result<(), Box<dyn Error>> {
    ...
    command
        ...
        .args(["run", "-p", "extractor-legacy", "--release"]);
    if checks_only {
