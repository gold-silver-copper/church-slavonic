...
| Independent registries | complete | 929 lexemes, 3,674 generated exact forms, 932 senses, typed principal parts, reviewed accent witnesses, 180 typed abbreviation cells, family decisions, positional, override, example, rejected-candidate, and reviewed-evidence tables |
...
| Noun/adjective productive morphology | broad but not language-wide | thirty-eight noun contracts exhaust Alypy §§32–44 regular and named irregular families plus the §8 `ц` spelling contract, including mobile-`е` `младенецъ` and distinct masculine/feminine `-іа` names; caller specifications can express arbitrary ordered irregular variants and typed defects; adjective coverage includes hard/soft/velar short and long positive forms, typed short-masculine stem alternants, complete long and short comparative inventories, full long superlatives, and the exceptional nine-cell predicate short-superlative agreement contract from independently reviewed stems |
...
| Independent verb systems | broad but not language-wide | regular principal-part bundles, exact irregular auxiliaries including the nine-cell simple future of `быти`, reviewed `дати`, productive tense/voice-specific declined participles and l-participles, and complete verbal nouns: `-їе` is formed from a past-passive platform while other §27 suffix families require an explicit complete noun; the distinct supine is source-closed as absent from the Russian/Synodal recension with a caller-exact interoperability seam |
...
| Abbreviation and printed orthography | partial | 180 individually reviewed contraction cells, reviewed accents and explicit positional operation; no blind/general contraction or inferred paradigm |
| Analyzer, capabilities, phrases, manifest lint | complete for registered data | multi-analysis reverse lookup, missing metadata, ten closed analytic-construction families with typed formation subtypes, and typed vocabulary lint without runtime data-format access |
| Deterministic reports and structural guards | complete | generated/extraction/evaluation/coverage freshness, typed-gap precedence, corpus hashes, recension, forbidden-authority, PUA, I/O, package, and witness guards, plus the synodal-gold full-enumeration gate |
| Leakage-resistant real evaluation | complete for the current registry; still statistically limited | 2,136 active passage-held-out morphology cells (2,136/2,136 Productive/Exploratory top-k) plus three historically preserved but correction-ledger-retracted `господи` misclassifications, 14 analytic phrase fixtures spanning the ten closed construction families, 74 typed contraction cases, 502 active masked-cell controls, 2,952-row exact-registry round-trip, held-out exact-attestation rows, inherited-cell, strict abstention, hostile-Unicode, and partition-contamination checks; accent and abbreviation witnesses use disjoint source partitions |
...
real-data regression tests, and explicit failures everywhere else. The next
language-wide checkpoint is the remaining liturgical accent, positional-
orthography, and abbreviation inventory, followed by lexical classification and
additional passage-disjoint evaluation—not speculative completion of
unsupported cells.
