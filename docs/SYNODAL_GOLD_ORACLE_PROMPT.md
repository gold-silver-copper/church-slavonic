...
- **Phase 1 (landed):** `docs/SYNODAL_GOLD_ORACLE.md` (normative comparison
  contract), the token oracle (`data/synodal/gold_token_oracle.tsv`, 58,467
  type rows, generated by `synodal-gold-oracle`), the Alypy paradigm oracle
  (`data/synodal/gold_paradigm_oracle.tsv`, `alypy-paradigm-oracle`), witness
  loaders, and the empty defect ledger `data/synodal/gold_source_defects.tsv`
  (commits 940595f, 4b8c1cd). The reviewed `synodal-evaluate` rows are folded
  into the token oracle's `confirmed_readings` column.
- **Phase 2 (landed, commit baa4ffa):** `cargo xtask synodal-gold
  --check/--fix` over both oracles; the first full gap committed as
  `reports/synodal-gold-gap.tsv`; wired into `check-structure` and CI.
- **Phase 3 (this change-set, 2026-08-26):** the wave machinery is retired.
  Deleted xtask commands and modules: `synodal-waves`, `synodal-wave-close`,
  `synodal-admit-check`, `synodal-type-holdout`, `synodal-evaluation-queue`,
  `synodal-family-review-queue`, `synodal-lexical-review-queue`,
  `synodal-marginal-recovery`, `synodal-accent-fit`, `synodal-coverage`
  (all modes; nothing surviving consumed its plain analysis output), and
  `synodal-predict` (its only input was the deleted coverage report; the gap
  file is the triage worklist). The standalone `synodal-evaluate` command is
  removed (rows folded into the oracle; the evaluation reports remain,
  regenerated by `synodal-regenerate`). Deleted ledgers and reports:
  `held_out_types.tsv`, `holdout_memorisation_baseline.tsv`,
  `coverage_floors.tsv`, `homonymy_allowlist.tsv`, `reports/synodal-waves.tsv`,
  all `synodal-coverage*` reports, `synodal-accent-fit.*`,
  `synodal-marginal-recovery.*`, the three review-queue report pairs, and the
  two prediction reports. Kept: `training_passages.tsv` and
  `coverage_passages.tsv` (still consumed by the surviving
  partition-disjointness guard, the extraction report, and the dictionary CLI
  coverage test), `evaluation.tsv` and friends (curated data),
  `synodal-lexical-union`, `synodal-fixture-bootstrap`, the sources/checksum
  machinery, and the immutable archive — which now also pins the wave-era
  closing docs `docs/history/SYNODAL_100_PERCENT_TOP_K_BASELINE.md` and
  `docs/history/SYNODAL_ACCENT_PARADIGM_FIT.md` as the closing entry recording
  that the wave program ended and this gate replaced it. Guard witnesses:
  the wave-ledger ratchet and admission-preflight witnesses are replaced by
  gold-gate witnesses (a tampered token oracle fails its committed
  self-validation; a gap row deleted from the committed worklist is reported
  as a regression by the full replay); the registry-staleness and archive
  tamper witnesses survive. CI's "Wave-close gates" step is replaced by the
  individually surviving checks (check-structure, guard witnesses, lexical
  union, fixture bootstrap, archive).
