...
| First-declension mixed `-цъ` masculine nouns | `SYN-NOUN-I-MIXED-TS-M-ALYPY-8-33-37` | Alypy §§8, 33–37 | citation in `-цъ` or mobile `-ецъ`, independent `ц`-final oblique stem, and masculine gender; `ц → ч` is restricted to the vocative and no mobile vowel is inferred outside the class |
...
| Reflexive voice | `SYN-VERB-REFLEXIVE-ALYPY-73` | Alypy §73 | a verb registered under a lemma in `-сѧ` stores bare stems and every generated cell receives the unaccented enclitic after accent realisation, with the host's final jer deleted; a surface in `-сѧ` with no registered reading analyses as the reflexive/passive voice of a registered active verb whose host form (jer restored) has a verbal reading, marked `reflexive` with the rule in its trace |
...
| Declined long active participles | `SYN-VERB-PARTICIPLE-{PRESENT,PAST}-ACTIVE-ALYPY-{95,96}` | Alypy §§95–96 | tense/voice-specific full-form stem and adjective class; a hard stem in `ш`/`щ`/`ж`/`ч` is declined with the Synodal mixed sibilant series (`SYN-ADJ-LONG-SIBILANT-ALYPY-57-58`: `-ихъ`/`-ими` never in `ы`, plural `-ымъ`/`-ыѧ` keep `ы`) |
...
The reviewed registry has 937 target lexemes and 940 reviewed senses. The
...
The locked Synodal lexical union is independently regenerated by
`cargo xtask synodal-lexical-union`. Its 1,258 source claims consist of all 229
...
- accent paradigms beyond the six reviewed lexical rules and complete
  breathing/positional-letter realization; all 48 Alypy §3.c abbreviation
  entries are already linked to the 55 typed semantic families; and
