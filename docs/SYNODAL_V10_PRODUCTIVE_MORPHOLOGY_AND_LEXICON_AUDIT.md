...
The matrix contains 56 reviewed system/subtype rows: 46 productive rows, 5 rows involving exact tables, 2 explicit irregular rows, and 3 unsupported rows. Counts describe engine contracts, not corpus forms or tokens.
...
- `SYN-NOUN-IV-F-OV-ALYPY-42-44` implements the complete `свекры` ending family without treating cell-specific `любовь` alternants as class-wide.
- `SYN-NOUN-IV-M-EN-ALYPY-42-44` implements the reusable masculine `-ен-` family without leaking lexeme-specific variants. `SYN-NOUN-IV-M-EN-KAMEN-ALYPY-43` adds only the cited `камень` alternatives; the collective `каменїе` remains separate.
...
The engine tests cover complete new noun paradigms, collective/ordinary plural separation, number restrictions, ordered variants, independent present edges and non-present stems, unified verb-system inventories, precise missing-formation diagnostics, provider conflicts and precedence, ordered batches, stable error codes, reusable accents, combining-mark order, and hostile Unicode. `data/synodal/linguistic_evaluation.tsv` adds eleven small source-linked contracts evaluated without frequency weighting.
