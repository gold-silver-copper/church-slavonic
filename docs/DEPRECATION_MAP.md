...
| `participle`, `participle_by_id`, `participle_with`, `participle_citation(_by_id)`, `participle_paradigm(_by_id)`, `ParticipleParadigm`, `Participle` declined cells | **Planned** — paradigm enumeration (declining a participle across case/number/gender feeds the citation back through the adjective machinery; designed, not built) |
...
| `compound_cardinal`, `compound_cardinal_with_one`, `compound_cardinal_with_options`, `compound_cardinal_paradigm`, `compound_cardinal_paradigm_with_one`, `compound_cardinal_paradigm_with_options` | **Dropped** — the six variants collapse per the ruthenian rule into one planned function, target signature `numeral(value: u16, case: Case, gender: Gender, animacy: Animacy) -> Result<RealizedPhrase, Error>` plus at most one options struct (absorbing the `_with_one` / `_with_options` axes); built in the phrases & numeral composition phase |
| `distributive_cardinal` and its five `_with_one`/`_with_options`/`_paradigm*` variants, `DistributiveCardinalParadigm`, `DistributiveCardinalOutcome` | **Dropped** — same collapse; distributivity becomes a field of the single options struct |
