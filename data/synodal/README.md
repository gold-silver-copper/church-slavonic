...
These rows were derived during the wave program by the retired
`synodal-accent-fit` command (methodology preserved in
`docs/history/SYNODAL_ACCENT_PARADIGM_FIT.md`); new rows are hand-authored
against the same validation, driven by `engine-wrong-accent` rows in the
gold gap (`reports/synodal-gold-gap.tsv`).
