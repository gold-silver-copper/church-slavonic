...
pub fn refresh_dictionary(dump: &Path, root: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let generated_dir = root.join("crates/old-church-slavonic-dictionary/generated");
    ...
    let generated = generated_rust(&senses);
    ...
    atomic_write_batch(&[
        ...
        (generated_dir.join("dictionary.rs"), generated.as_bytes()),
        (
...
pub fn check_dictionary(root: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let generated_path = root.join("crates/old-church-slavonic-dictionary/generated/dictionary.rs");
    if fs::read_to_string(&generated_path)? != generated_rust(&senses) {
        return Err(format!(
            "generated dictionary is stale: refresh {}",
    ...
    let package_sources_toml =
        fs::read_to_string(root.join("crates/old-church-slavonic-dictionary/SOURCE.toml"))?;
    ...
    if package_sources_toml != sources_toml {
        return Err("published dictionary SOURCE.toml is stale".into());
    }
...
fn generated_rust(senses: &[DictionarySenseRow]) -> String {
    let mut out = String::from(
        "// @generated by old-church-slavonic-extractor. Do not edit manually.\n\
         pub(crate) static SENSES: &[SenseRecord] = &[\n",
    );
    for sense in senses {
        out.push_str("    SenseRecord { ");
        out.push_str(&format!(
            "id: {:?}, source_sense_id: {:?}, lemma: {:?}, page_word: {:?}, key: {:?}, page_key: {:?}, part_of_speech: {:?}, inflection_lexeme_id: {:?}, ",
            sense.id,
            ...
            sense.inflection_lexeme_id.as_deref()
        ));
        out.push_str(&format!(
            "glosses: {}, raw_glosses: {}, tags: {}, topics: {}, examples: &[",
            string_slice(&sense.glosses),
            string_slice(&sense.raw_glosses),
            string_slice(&sense.tags),
            string_slice(&sense.topics),
        ));
        for example in &sense.examples {
            out.push_str(&format!(
                "ExampleRecord {{ text: {:?}, romanization: {:?}, translation: {:?}, reference: {:?} }},",
                example.text, example.romanization, example.translation, example.reference
            ));
        }
        out.push_str("] },\n");
    }
...
mod tests {
    ...
    #[test]
    fn generated_strings_are_valid_rust_literals() {
        assert_eq!(
            string_slice(&["gold coin".to_string(), "food".to_string()]),
            "&[\"gold coin\", \"food\"]"
        );
    }
}
