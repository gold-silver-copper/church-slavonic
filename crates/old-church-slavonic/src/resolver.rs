...
fn resolve_queried_noun(query: &str, cell: NounCell) -> Result<FormSet, InflectionError> {
    ...
    match candidates.as_slice() {
        [one] => {
            ...
            queried_result(query, record, noun_by_id(record.id, cell))
        }
        [] => Err(InflectionError::unknown_lemma(query, PartOfSpeech::Noun)),
...
pub fn noun(lemma: &str, cell: NounCell) -> Result<FormSet, InflectionError> {
    resolve_queried_noun(lemma, cell)
}
...
fn reviewed_twofold_noun_form(
    ...
    let direct = matches!(
        member.canonical_lemma(),
        "дѣлател҄ь" | "гражданинъ" | "рабын҄и" | "сѫдии"
    ) && matches!(
        cell.case,
        Case::Nominative | Case::Accusative | Case::Locative
    );
    let form = FormVariant {
        text: orthography::canonical_display(&predicted.text)?,
        romanization: None,
...
pub fn noun_paradigm_by_id(id: &str) -> Result<NounParadigm, InflectionError> {
    let (id, lemma) = noun_identity_from_id(id)?;
...
pub fn reviewed_cardinal_numeral(
    ...
    old_church_slavonic_core::numeral::cardinal_form_set(identity, cell)
}
...
pub fn reviewed_cardinal_magnitude(
    ...
    old_church_slavonic_core::numeral::magnitude_form_set(identity, cell)
}
...
fn reviewed_numeral_variants(
    ...
    old_church_slavonic_core::numeral::numeral_variants_form_set(
        lemma,
        authority,
        source_feature,
        variants,
...
pub fn compound_cardinal_with_options(
    ...
    old_church_slavonic_core::numeral::compound_cardinal(value, cell, options)
}
...
pub(crate) fn build_compound_cardinal_paradigm(
    ...
    old_church_slavonic_core::numeral::validate_cardinal_composition_options(options)?;
...
pub fn distributive_cardinal_with_options(
    ...
    old_church_slavonic_core::numeral::distributive_cardinal(value, cell, options)
}
...
pub(crate) fn reviewed_grammar_token(
    ...
    old_church_slavonic_core::numeral::grammar_token(text, rule_id, source_feature, authority)
}
...
mod tests {
    ...
    #[test]
    fn exact_noun_cell_precedes_a_conflicting_reviewed_deformation() {
        let forms = noun(
            "дѣлател҄ь",
            NounCell {
                case: Case::Nominative,
        ...
        .expect("dictionary member with a reviewed class");
        assert_eq!(forms.primary_text(), "дѣлател҄и");
        assert_eq!(forms.source(), &FormSource::DictionaryTable);

        let source_only = noun(
            "рабын҄и",
            NounCell {
        ...
        .expect("source-only reviewed noun");
        assert_eq!(source_only.primary_text(), "рабын҄ѭ");
        assert_eq!(
            source_only.source(),
