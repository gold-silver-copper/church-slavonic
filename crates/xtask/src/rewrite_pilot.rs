...
fn encode_verb_metadata(
    metadata: &crate::ocs_verb_metadata::DictionaryVerbMetadata,
) -> VerbMetaCodes {
    VerbMetaCodes {
        aspect: metadata
            .aspect
...
fn load_verb_oracle(root: &Path) -> Result<VerbOracle, Box<dyn Error>> {
    ...
    for lexeme in &registry.lexemes {
        ...
        let codes = crate::ocs_verb_metadata::verb_metadata_from_registry(&registry, &lexeme.id)
