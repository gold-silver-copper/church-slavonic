//! Typed offline view of the extracted verb principal-part metadata.
//!
//! Ported from the retired `old-church-slavonic` facade's `metadata` module
//! (see git history) so the rewrite-pilot oracles can keep validating the
//! extracted verb metadata rows without the deleted facade. The generated
//! `VERB_METADATA` table the facade read was emitted from the same
//! `data/extracted` registry rows this module now consumes directly.
...
pub(crate) fn verb_metadata_from_registry(
    ...
    let fields = registry
        ...
        .map(|row| NormalizedVerbMetadataField {
            ...
            source_feature: row.source_feature.clone(),
            source_form: row.source_form.clone(),
    ...
    DictionaryVerbMetadata::from_normalized_fields(id, lexeme.lemma.clone(), fields)
}
