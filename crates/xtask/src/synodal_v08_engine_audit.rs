...
fn render(root: &Path) -> Result<String, Box<dyn Error>> {
    ...
    out.push_str("The engine now productively covers thirty-eight reviewed noun contracts, including all regular tables and every reusable or lexeme-bounded exception named in Alypy §§32–44 plus the §8 `ц` spelling contract. Cell-scoped `любовь` / `церковь`, paired `ѻко` / `ꙋхо`, syncopating `день`, mobile-`е` `младенецъ`, mixed `господь`, gendered `-іа` names, ethnonyms in `-инъ`, invariant Hebrew loans, and the `ꙋдъ : ꙋдес-` analogy are complete without permitting arbitrary stem cross-products.\n\n");
    ...
    out.push_str("`NounSpec`, `AdjectiveSpec`, and `VerbSpec` accept closed linguistic types, validated Unicode stems and principal parts, explicit provenance, optional irregular/defective cells, and typed `AccentParadigm` plus `PositionalParadigm` presentation metadata. Fourth-declension nouns require the independently supplied extended stem; `NounNumberInventory` makes absent numbers explicit. `PresentPrincipalParts` and `VerbSpecBuilder::present_series` install the three independent present inputs atomically. Registry, provider, and explicit routes delegate to the same pure productive kernel after identity and override layers.\n\n");
    ...
    out.push_str("- `SYN-NOUN-I-MIXED-TS-M-ALYPY-8-33-37` implements `младенецъ : младенц-` with an independently validated mobile vowel, complete `ц`-class endings, and the cell-scoped `ц → ч` vocative.\n");
    ...
    out.push_str("- `SYN-ADJ-SHORT-VELAR-ALYPY-53-57` and `SYN-ADJ-LONG-VELAR-ALYPY-57` implement the complete positive velar table with cell-scoped first/second palatalization. `SYN-ADJ-SHORT-MASCULINE-STEM-ALYPY-52` adds typed `-нн-` reduction and mobile-`е` citation stems without changing the remaining agreement cells.\n");
