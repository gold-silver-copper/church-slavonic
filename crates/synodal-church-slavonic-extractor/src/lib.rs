...
use synodal_church_slavonic_core::{Animacy, GrammarCell, Number, RenderedText, SynodalWord};
...
fn validate_noun_restriction_exact_forms(
...
fn grammar_cell_matches_part_of_speech(cell: GrammarCell, part_of_speech: &str) -> bool {
    matches!(
        (cell, part_of_speech),
...
fn validate_positional_paradigms(path: &Path, table: &Table) -> Result<()> {
    let mut ids = BTreeSet::new();
    for (offset, row) in table.rows.iter().enumerate() {
        let line = offset + 2;
        validate_target(path, line, &row[8])?;
...
fn validate_positional_operation_code(path: &Path, line: usize, value: &str) -> Result<()> {
    if matches!(
        value,
