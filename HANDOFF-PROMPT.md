# Handoff: church-slavonic 3.3.0 (tag `v3.3.0`)
...
Parts 0–2 are executed as 3.3.0 (tag `v3.3.0`), its Parts 3–4 are the
plan for 3.4.0; the open list below is what the releases left. The
...
- `crates/church-slavonic-tools` — `cargo xtask`: `import <source> --pos
  ...
  `train-tagger [--epochs n]`, `analyze [--ocs] <word>…`, `titlo
...
pronouns 98.07%. Bible treebank (631,946 tokens): one cell 240,672
(38.1%), sets 2,311 (0.4%), tagger 190,494 (30.1%), closed 179,009
(28.3%), several lexemes 12,880 (2.0%), verbatim 5,430 (0.9%),
...
names, compounds). Hand overlay: 337 verses, 3,757 leaves; the rules
exclude no hand cell (np-agree 179/179, prep-gov 142/145, subj-verb
31/31, voc-drop 50/69, one-subject 10/10 resolved; 459 in all); the
tagger 74.9% (1,228 of 1,639). The tables are in `CHANGELOG.md` under
"3.3.0", "3.1.0" and "3.0.0" and in the README.
...
1. **`V3.3-PROMPT.md` Parts 3–4 (3.4.0)**: the verb that is several
   lexemes (bare-loc/bare-voc eliminations: the census found 135 such
   tokens — scale the part to what the overlay can prove), then the
   tagger's transfer measurement (five folds of the overlay, no model
   shipped).
2. **What 3.3 left of the verbatim residue** (`census verbatim`, the
...
3. **What the clause rule cannot reach** (3.2 Part 5): a clause whose
...
4. **What the homonymy layers left** (`docs/OPEN-DESIGNS.md` 1b, executed
   ...
   poetry, the Epistles).
5. **What 2.2 left.** Kaikki's aorist and l-participle junk is counted,
